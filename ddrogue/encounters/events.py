...
from ..colors import BLACK, LIGHT_BLUE, GREY, WHITE
from ..constants import UI_SIZE, SAVE_DIR, SCREEN, FONT
from ..ui import menu, select_loop, Selectable
from ..mechanics.dice import roll
from .combat import (move_to, FULL_ROUND_ACTIONS, STANDARD_ACTIONS,
                     MOVE_ACTIONS, SWIFT_ACTIONS, FREE_ACTIONS)
...
def ability(state, event):
    ...
    if not ability:
        state._print('No ability selected')
        return
...
def interact_with_user(state):
    """
    Get an event, returning the event if it's unrelated to the UI.
    """
    # Wait for an event
    ...
    if event.type == pygame.KEYUP:
        ...
        try:
            ...
            print(e)
        if event.key == ord('\t'):  # tab
            # switch to the next ui element
            state.selected_element += 1
            if state.selected_element >= len(state.ui):
                state.selected_element = 0
            elif state.selected_element < 0:
                state.selected_element = len(state.ui) - 1
            return
        elif event.key in [ord(c) for c in '\n ']:  # space /enter
            state.output.fullscreen()
        elif event.key == 275:  # right
            state.hud.adjust_selected_player(1)
        elif event.key == 276:  # left
            state.hud.adjust_selected_player(-1)
    elif event.type in [pygame.MOUSEMOTION, pygame.MOUSEBUTTONUP]:
        if state.output.area_rect.collidepoint(event.pos):
            print('output box collision', state.output.rect)
            state.output.fullscreen()
        elif state.hud.area_rect.collidepoint(event.pos):
            print('hud collision', state.hud.rect)
            for index in range(len(state.hud.elements)):
                if state.hud.element_areas[index].collidepoint(event.pos):
                    print('clicked on ', index)
                    state.hud.selected_element = index
                    func = state.hud.element_functions.get(index, None)
                    if func:
        ...
        return
    # Removed event checks because they just return the event
    # if event.type == pygame.KEYUP:
    #     if event.key == 27:  # esc
...
def player_turn(state):
    ...
    while not done:
        # Wait for an event
        while 1:
            event = interact_with_user(state)
            if event:
                break
        if event == 'done':
            done = 1
...
class EncounterState:
    def __init__(self, players, npcs, floor_plan, keymap_file):
        ...
        self.map = EncounterMap(
            self.actors, floor_plan,
            (s_width - UI_SIZE, s_height - UI_SIZE / 2)
        )
        self.hud = HUD(
            self, self.players, (s_width - UI_SIZE, 0), UI_SIZE, s_height
        )
        self.output = StatusBox(
            self, (0, s_height - UI_SIZE / 2), s_width - UI_SIZE, UI_SIZE / 2
        )
        ...
        # If the first character to act is a pc, this is necessary
